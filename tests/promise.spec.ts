...
describe("Promise", () => {
  ...
  describe('should resolve', () => {
    ...
    it("when then'ed 2 times", () => {
      expect.assertions(1);
      Promise.resolve('a').then(v => v + 'b').then(v => expect(v).toBe('ab'));
      startPromiseLoop();
    });
  });
  ...
  describe('should catch', () => {
    it('rejection', () => {
      expect.assertions(1);
      Promise.reject('a').then(v => v + 'b').catch(v => v + 'c').then(v => expect(v).toBe('ac'));
      startPromiseLoop();
    });

    it('rejection and then', () => {
      expect.assertions(1);
      Promise.reject('a').catch(v => v + 'c').then(v => v + 'b').then(v => expect(v).toBe('acb'));
      startPromiseLoop();
    });
  });
